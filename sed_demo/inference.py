...
class AudioModelInference:
    ...
    def __init__(
        ...
        #
        self.mel_filt = librosa.filters.mel(
            sr=samplerate, n_fft=winsize, n_mels=n_mels, fmin=mel_fmin, fmax=mel_fmax
        )
    ...
    def __call__(self, wav_arr):
        ...
        with torch.no_grad():
            logmel_spec = torch.from_numpy(logmel_spec.astype(np.float32)).unsqueeze(
                0
            )  # (1, t, nbins)
...
class PredictionTracker:
    ...
    def __init__(self, all_labels, allow_list=None, deny_list=None):
        ...
        if deny_list is None:
            deny_list = []
        self.labels = [l for l in all_labels if l in allow_list and l not in deny_list]
