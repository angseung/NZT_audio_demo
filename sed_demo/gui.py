...
class ResponsiveImgCanvas(tk.Canvas):
    ...
    def __init__(self, parent, img_path, margin_ratio=1.1, max_height=None, **kwargs):
        """
    ...
    def on_resize(self, event):
        ...
        """
        self.resized_img, (_, new_h) = self.get_new_width_img(event.width, self.max_h)
...
class ResponsiveImgBar(ttk.Frame):
    ...
    def __init__(self, parent, img_paths, max_height=None, img_pad=0, bg=None):
        ...
        self.imgs = [
            ResponsiveImgCanvas(
                self, lp, max_height=max_height, bd=0, bg=bg, highlightthickness=0
            )
            for lp in img_paths
        ]
...
class DemoFrontend(tk.Tk):
    ...
    def __init__(
        ...
        self.top_area, self.top_widgets = self.get_top_area(
            max_top_banner_h, padding=(margin, margin, margin, 0)
        )  # ltrb
        ...
        self.style.configure(
            "TButton",
            font=("Helvetica", table_fontsize),
            borderwidth=1,
            background=self.BUTTON_COLOR,
        )
        self.style.configure("TFrame", background=self.BG_COLOR)
        self.style.configure("TCanvas", background=self.BG_COLOR)
        self.style.configure(self.BAR_STYLE, relief="sunken", background=self.BAR_COLOR)
    ...
    def get_top_area(self, max_img_h=150, padding=(0, 0, 0, 0)):
        ...
        title_lbl = ttk.Label(
            top_area, text=self.TITLE, style="title.TLabel", padding=(0, 0, 0, 20)
        )
    ...
    def get_mid_area(self, num_items, padding=(0, 0, 0, 0)):
        ...
        rank_lbl = ttk.Label(
            mid_area, text="Rank", style="tablehead.TLabel", padding=(0, 0, 0, 5)
        )
        sound_lbl = ttk.Label(mid_area, text="Sound", style="tablehead.TLabel")
        confidence_lbl = ttk.Label(
            mid_area, text="Confidence", style="tablehead.TLabel"
        )
        ...
        for i in range(1, num_items + 1):
            rank = ttk.Label(
                mid_area, text=f"{i}", padding=(0, 5, 0, 0), style="tablerow.TLabel"
            )
        ...
        bottom_slack.grid(
            row=num_items + 1, column=0, columnspan=3, sticky=(tk.N, tk.S, tk.E, tk.W)
        )
        mid_area.rowconfigure(num_items + 1, weight=1)
        #
        widgts = {
            "labels": [rank_lbl, sound_lbl, confidence_lbl],
            "ranks": ranks,
            "sounds": sounds,
    ...
    def get_bottom_area(self, max_logos_h=50, img_padding=0):
        ...
        logo_bar = ResponsiveImgBar(
            bottom_area, self.logo_paths, max_logos_h, img_padding, bg=self.BG_COLOR
        )
        #
        exit_but.grid(row=0, column=0, columnspan=3)
        logo_bar.grid(row=1, column=0, columnspan=3, sticky=tk.S)
        #
